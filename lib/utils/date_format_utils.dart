...
class MyDateUtil {
  ...
  static String formatDate(DateTime? dateTime, {String? format}) {
    ...
    format = format ?? MyDateFormat.full;
  ...
  static bool isToday(int? milliseconds, {bool isUtc = false, int? locMs}) {
    ...
    DateTime now;
    if (locMs != null) {
      now = MyDateUtil.getDateTimeByMs(locMs);
    } else {
  ...
  static bool isWeek(int? ms, {bool isUtc = false, int? locMs}) {
    ...
    DateTime _now;
    if (locMs != null) {
      _now = MyDateUtil.getDateTimeByMs(locMs, isUtc: isUtc);
    } else {
  ...
  static String millis2FitTimeSpan(int millis, int precision) {
    ...
    var unitLen = <int>[86400000, 3600000, 60000, 1000, 1];
  ...
  static getDateByDays(int days) {
    DateTime now = DateTime.now();
    DateTime thirtyDaysAgo = now.subtract(Duration(days: days));
    String sss = formatDate(thirtyDaysAgo, format: MyDateFormat.full);
    return sss;
  }
}
